...
st.title("Analisi e Interpretabilità del Ranking FT - SHAP Values")
...
try:
    ...
    st.success("Dati caricati, modello addestrato e valori SHAP calcolati con successo!")
...
with tab_esplorazione:
    st.header("Tabella di input e Visualizzazioni Esplorative dei Dati")
...
with tab_drilldown:
    ...
    with col_force:
        ...
        st.markdown("#### Force Plot (Interattivo)")
    ...
    st.markdown("---")
    st.subheader("Analisi Competitor per Waterfall Plot e Confronto Dati")
    ...
    if selected_competitors:
        ...
        st.markdown("#### Tabella di Confronto (Università di riferimento vs Competitor Selezionati)")
        ...
        # FIX: Prima di impostare 'name' come indice, assicurati che sia unico, 
        # rimuovendo eventuali duplicati nell'input (che non dovrebbero esserci 
        # se i dati sono puliti, ma è una misura di sicurezza).
        df_comparison_temp = df[df['name'].isin(uni_names_to_compare)].drop_duplicates(subset=['name'])
        
        # Imposta l'indice a 'name'. Se anche qui non è unico, 
        # c'è un problema grave con i dati sorgente.
        df_comparison = df_comparison_temp.set_index('name', verify_integrity=True) 
        
        # Seleziona le colonne da mostrare per il confronto
        
        # Calcola le feature più importanti in media (Top 5)
        ...
        unique_model_features = [
            col for col in all_model_features if col not in initial_cols
        ]
        ...
        numeric_feature_cols = [c for c in unique_top_5_features if df_comparison_fixed[c].dtype != 'object']
        for col in numeric_feature_cols:
             df_comparison_fixed[col] = df_comparison_fixed[col].apply(lambda x: f"{x:.2f}")

        # La colonna 'location' è già una stringa se l'hai definita come tale.
        
        # Creazione della tabella trasposta per una migliore leggibilità
        # Tutte le colonne sono ora stringhe.
        df_display = df_comparison_fixed.T
        df_display.columns.name = 'Feature'

        # Styling e formattazione
        # Rimuoviamo il metodo .format() qui, poiché i dati sono già formattati come stringhe
        ...
        st.markdown("#### Waterfall Plot SHAP per Competitor")
        ...
        n_cols_waterfall = 2 if len(selected_competitors) > 1 else 1 # Mostra 2 colonne se ci sono almeno 2 competitor
        waterfall_cols = st.columns(n_cols_waterfall)
        
        # Itera sui competitor e genera i plot
        for i, competitor_name in enumerate(selected_competitors):
